...
def triangle_answer(base: int, height: int, hypotenuse: int,is_right: bool, operation: str, student_result: float):
    """Checks if the student's input matches the mathematically correct answer."""
    if operation == "area":
        triangle_correct_answer = base * height * 0.5
    else:
        if is_right:
            triangle_correct_answer = base + height + hypotenuse
        else:
            triangle_correct_answer = base + 2 * hypotenuse

    if math.isclose(triangle_correct_answer, student_result, abs_tol=0.1):
        is_correct = True
    else:
        is_correct = False

    return triangle_correct_answer, is_correct
...
if __name__ == "__main__":
    ...
    while True:
        ...
        triangle_correct_answer, is_correct = triangle_answer(base, height, hypotenuse,is_right, operation, ans_val)
